...
<div class="d-flex justify-content-end gap-2" style="margin-right: 100px;">
    ...
    @if (!Auth::guard('organization')->check()) <!-- ✅ Chỉ hiện nếu không phải organization -->
        @if ($hasFollowed)
            <form method="POST" action="{{ route('unfollow.organization', $organization->organization_id) }}">
                @csrf
                <button type="submit" class="btn btn-secondary">
                    <i class="fas fa-minus-circle me-1"></i>Đã theo dõi
                </button>
            </form>
        @else
            <form method="POST" action="{{ route('follow.organization', $organization->organization_id) }}">
                @csrf
                <button type="submit" style="background-color: #ff5722; color: white; border: none; padding: 8px 16px; border-radius: 4px;">
